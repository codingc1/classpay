...
return(
  <div className="flex flex-col items-center ">
    ...
    <div className="w-full max-w-2xl flex flex-col justify-center lg:mt-10">
      ...
      <h2>이 약관은 학급 페이에서 제공하는 인터넷 관련 서비스(이하 “서비스”라 한다)를 이용함에 있어 사이버 몰과 이용자의 권리․의무 및 책임사항을 규정함을 목적으로 합니다.</h2>
      <br></br>

      <h1 className="font-bold">제2조(정의) </h1>
      <h2>① “이용자”란 학급 페이 제공하는 서비스를 받는 회원 및 비회원을 말합니다.</h2>
      <h2>② ‘회원’이라 함은 학급 페이에 회원등록을 한 자로서, 계속적으로 학급 페이가 제공하는 서비스를 이용할 수 있는 자를 말합니다.</h2>
      <h2>③ ‘비회원’이라 함은 회원에 가입하지 않고 학급 페이가 제공하는 서비스를 이용하는 자를 말합니다.</h2>
      <br></br>

      <h1 className="font-bold">제3조 (약관 등의 명시와 설명 및 개정)  </h1>
      <h2>① 학급 페이는 「전자상거래 등에서의 소비자보호에 관한 법률」, 「약관의 규제에 관한 법률」, 「전자문서 및 전자거래기본법」, 「전자금융거래법」, 「전자서명법」, 「정보통신망 이용촉진 및 정보보호 등에 관한 법률」, 「방문판매 등에 관한 법률」, 「소비자기본법」 등 관련 법을 위배하지 않는 범위에서 이 약관을 개정할 수 있습니다.</h2>
      ...
      <h2>③ 사업종목의 전환, 사업의 포기, 업체 간의 통합 등의 이유로 서비스를 제공할 수 없게 되는 경우에는 이용자에게 통지하고 당초 “몰”에서 제시한 조건에 따라 소비자에게 보상합니다. 보상기준 등을 고지하지 아니한 경우에는 이용자들의 마일리지 또는 적립금 등을 학급 페이에서 통용되는 통화가치에 상응하는 현물 또는 현금으로 이용자에게 지급합니다.</h2>
      <br></br>

      <h1 className="font-bold">제5조(회원가입)  </h1>
      <h2>① 이용자는 학급 페이에서 정한 가입 양식에 따라 회원정보를 기입한 후 이 약관에 동의한다는 의사표시를 함으로서 회원가입을 신청합니다.</h2>
      <h2>② 은 제1항과 같이 회원으로 가입할 것을 신청한 이용자 중 다음 각 호에 해당하지 않는 한 회원으로 등록합니다.</h2>
      <h2> 1. 가입신청자가 이전에 회원자격을 상실한 적이 있는 경우, 다만 제7조제3항에 의한 회원자격 상실 후 학급 페이의 회원재가입 승낙을 얻은 경우에는 예외로 한다.</h2>
      <h2> 2. 등록 내용에 허위, 기재누락, 오기가 있는 경우</h2>
      <h2> 3. 기타 회원으로 등록하는 것이 학급 페이의 기술상 현저히 지장이 있다고 판단되는 경우</h2>
      <h2>③ 회원가입계약의 성립 시기는 학급 페이의 승낙이 회원에게 도달한 시점으로 합니다.</h2>
      <h2>④ 회원은 회원가입 시 등록한 사항에 변경이 있는 경우, 상당한 기간 이내에 학급 페이에 대하여 회원정보 수정 등의 방법으로 그 변경사항을 알려야 합니다.</h2>
      <br></br>

      <h1 className="font-bold">제6조(회원 탈퇴 및 자격 상실 등) </h1>
      <h2>① 회원은 학급 페이에 언제든지 탈퇴를 요청할 수 있으며 학급 페이는 회원탈퇴를 처리합니다.
      탈퇴시 해당 메일로 재가입을 막기 위해 아이디 이메일 주소, 닉네임은 보관합니다.</h2>
      <h2> ② 회원이 다음 각 호의 사유에 해당하는 경우, 학급 페이는 회원자격을 제한 및 정지시킬 수 있습니다.</h2>
      <h3>1. 가입 신청 시에 허위 내용을 등록한 경우</h3>
      <h3>2. 학급 페이를 이용하여 구입한 재화 등의 대금, 기타 학급 페이 이용에 관련하여 회원이 부담하는 채무를 기일에 지급하지 않는 경우</h3>
      <h3>3. 다른 사람의 학급 페이 이용을 방해하거나 그 정보를 도용하는 등 질서를 위협하는 경우</h3>
      <h3>4. 학급 페이를 이용하여 법령 또는 이 약관이 금지하거나 공서양속에 반하는 행위를 하는 경우</h3>
      ...
      <h2>① 학급 페이는 이용자의 개인정보 수집시 서비스제공을 위하여 필요한 범위에서 최소한의 개인정보를 수집합니다. </h2>
      <h2>② 학급 페이는 이용자의 개인정보를 수집·이용하는 때에는 당해 이용자에게 그 목적을 고지하고 동의를 받습니다. </h2>
      <h2>③ 학급 페이는 수집된 개인정보를 목적외의 용도로 이용할 수 없으며, 새로운 이용목적이 발생한 경우 또는 제3자에게 제공하는 경우에는 이용·제공단계에서 당해 이용자에게 그 목적을 고지하고 동의를 받습니다. 다만, 관련 법령에 달리 정함이 있는 경우에는 예외로 합니다.</h2>
      ...
      <h1 className="font-bold">제9조(학급 페이의 의무) </h1>
      ...
      <h2>③ 회원이 자신의 ID 및 비밀번호를 도난당하거나 제3자가 사용하고 있음을 인지한 경우에는 바로 학급 페이에 통보하고 학급 페이의 안내가 있는 경우에는 그에 따라야 합니다.</h2>
      <br></br>

      <h1 className="font-bold">제11조(이용자의 의무) 이용자는 다음 행위를 하여서는 안 됩니다. </h1>
      <h2>1. 신청 또는 변경시 허위 내용의 등록</h2>
      <h2> 2. 타인의 정보 도용</h2>
      <h2>3. 학급 페이에 게시된 정보의 변경</h2>
      <h2>4. 학급 페이가 정한 정보 이외의 정보(컴퓨터 프로그램 등) 등의 송신 또는 게시</h2>
      <h2>5. 학급 페이 기타 제3자의 저작권 등 지적재산권에 대한 침해</h2>
      <h2>6. 학급 페이 기타 제3자의 명예를 손상시키거나 업무를 방해하는 행위</h2>
      <h2>7. 외설 또는 폭력적인 메시지, 화상, 음성, 기타 공서양속에 반하는 정보를 몰에 공개 또는 게시하는 행위</h2>
      <h2>8. 과도한 학급 페이 사용</h2>
      <br></br>

      <h1 className="font-bold">제12조(저작권의 귀속 및 이용제한) </h1>
      <h2>① 학급 페이가 작성한 저작물에 대한 저작권 기타 지적재산권은 학급 페이에 귀속합니다.</h2>
      <h2>② 이용자는 학급 페이를 이용함으로써 얻은 정보 중 학급 페이에게 지적재산권이 귀속된 정보를 “학급 페이의 사전 승낙 없이 복제, 송신, 출판, 배포, 방송 기타 방법에 의하여 영리목적으로 이용하거나 제3자에게 이용하게 하여서는 안됩니다.</h2>
      <h2>③ 학급 페이는 약정에 따라 이용자에게 귀속된 저작권을 사용하는 경우 당해 이용자에게 통보하여야 합니다.</h2>
      <br></br>
      ...
      <h2>① 학급 페이와 이용자 간에 발생한 전자상거래 분쟁에 관한 소송은 제소 당시의 이용자의 주소에 의하고, 주소가 없는 경우에는 거소를 관할하는 지방법원의 전속관할로 합니다. 다만, 제소 당시 이용자의 주소 또는 거소가 분명하지 않거나 외국 거주자의 경우에는 민사소송법상의 관할법원에 제기합니다.</h2>
      <h2>② 학급 페이와 이용자 간에 제기된 전자상거래 소송에는 한국법을 적용합니다.</h2>
