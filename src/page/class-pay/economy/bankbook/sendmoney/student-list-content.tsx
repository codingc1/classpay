...
export const StudentListContent=({setIsModal}:{setIsModal:React.Dispatch<React.SetStateAction<boolean>>}) => {
    ...
    const {transWidth, windowSize} = useWindowSizeTrans()
    ...
    <div className=" box-border" style={{width:CSS_LEN.popup.wide+'px',}} >
        <div className=" mt-3 grid box-border  " style={{width:(CSS_LEN.popup.wide-20)+'px', height:`${(popupHeight()-30)+'px'}`, overflowY: 'scroll',
...
return(
    <PopupCenterStudent onClose={popupClose} contents={contents} option={{width:transWidth(CSS_LEN.popup.wide), height:popupHeight()}} isTopClose={false} />
)
}