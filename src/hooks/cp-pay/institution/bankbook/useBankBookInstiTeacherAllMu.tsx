...
import { CP_TEACHER_GETSTUDENTS_ALL_BANKBOOKS_DAY_MUTATION } from "../../trade/cp-bill";
...
export const useBankBookTeacherAllMu = ({setData,setIsLoading}:{ setData?:any,setIsLoading?:any}) => {
    ...
    const [cp_MyBanksMonthMutation, { loading,  }] = useMutation<cp_teacherGetBankBookAllMutationMutation, cp_teacherGetBankBookAllMutationMutationVariables>(CP_TEACHER_GETSTUDENTS_ALL_BANKBOOKS_DAY_MUTATION, {async onCompleted (data){
        
        const resultData = data.cp_teacherGetBankBookAll
        resultSetData(resultData )
        //add => teacherAllStudentsAdd 키가 다름
        editCpPayVar.bankBook.teacherAllStudentsAdd({year:date.current.year, month:date.current.month,day:date.current.day, data:resultData})
